...
def replace_readme():
    ...
    with open(os.path.join(os.getcwd(),"EditREADME.md"),'r',encoding='utf8') as load_f:
        ...
        before_info_list =  re.findall(r'\*\s\[RSS源\]\(.*\)\s+\{\{latest_content\}\}' ,edit_readme_md)
        # 填充统计RSS数量
        ...
        for before_info in before_info_list:
            # 获取link
            link = re.findall(r'\[RSS源\]\((.*)\)', before_info)[0]
            # 生成超链接
            ...
            if(len(rss_info) == 0):
                ...
                rss_item_num=0
                with open(os.path.join(os.getcwd(),"README.md"),'r',encoding='utf8') as load_f_old:
    ...
    # 将新内容
    with open(os.path.join(os.getcwd(),"README.md"),'w',encoding='utf8') as load_f:
